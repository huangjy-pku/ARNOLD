...
def action_interpolation(trans_previous, rotation_previous, trans_target, rotation_target, alphas, task):
    action_list = []
    ...
    for eval_split in EVAL_SPLITS:
        ...
        while len(data) > 0:
            ...
            gt_actions = [
                ...
                gt_frames[3]['position_rotation_world'] if 'water' not in args.task \
            ...
            for step in range(horizon):
                ...
                if current_target is None:
                    ...
                    if stage == 0:
                        ...
                        if position_rotation_interp_list is None:
                            if use_gt[1]:
                        ...
                        if NEED_INTERPOLATION[args.task] and 'water' not in args.task:
                            if position_rotation_interp_list is None:
                                alphas = np.linspace(start=0, stop=1, num=NUM_INTERPOLATION)[1:]
                                position_rotation_interp_list = action_interpolation(
                                    trans_pick, rotation_pick, trans_target, rotation_target, alphas, args.task
                                )
                        ...
                        elif 'water' in args.task:
                            # lift up cup
                            current_target = (
                                np.array([trans_pick[0], trans_target[1], trans_pick[2]]), rotation_pick, grip_open
                            )
                        else:
                            current_target = (trans_target, rotation_target, grip_open)
                    
                    elif stage == 3:
                        # move to the position before pouring water
                        current_target = (trans_target, rotation_pick, grip_open)
                        ...
                        if position_rotation_interp_list is None:
                            ...
                            position_rotation_interp_iter = iter(position_rotation_interp_list)
                        if False: #pour_terminated(scene_loader, simulation_context):
                            # predictor emits termination signal
                            position_rotation_interp_list = None
                            stage += 1
                            continue
                        else:
